...
import { requireDb, getItemBySlug, getBazaarProduct } from '$lib/server/db';
...
export const entries: EntryGenerator = () => catalogEntries('bazaar');

export const load: PageServerLoad = async ({ params, platform, setHeaders }) => {
	setHeaders({ 'cache-control': MARKET_PAGE_CACHE });
	const known = catalogItem('bazaar', params.slug);
	if (known) return known;

	// Items added after the last build get the same shell on demand.
	const db = requireDb(platform);
	const item = await getItemBySlug(db, params.slug);
	if (!item) error(404, 'Unknown product');
	const market = await getBazaarProduct(db, item.id);
	if (!market) error(404, 'Unknown product');
