...
import { writeBazaarRun, writeAuctionRun, rollupAll, pruneStaleSnapshots, assertPopulated } from '../src/db';
import { bucketMedian, bazaarMedian, auctionMedian, RAW_WINDOW, HOURLY_WINDOW, DAY } from '../../../src/lib/market/bucket';
...
describe('writeAuctionRun', () => {
	test('writes snapshot, points, meta, and upserts official item metadata', async () => {
		const stats: AuctionItemStats = { name: 'Hyperion', tier: 'LEGENDARY', lowestBin: 90_000_000, medianBin: 95_000_000, count: 12 };
		...
		const item = await env.DB
			.prepare("SELECT slug, name, tier, category, npc FROM items WHERE id='HYPERION'")
			.first<{ slug: string; name: string; tier: string; category: string; npc: number }>();
		expect(item).toEqual({ slug: 'hyperion', name: 'Hyperion', tier: 'LEGENDARY', category: 'SWORD', npc: 1_000_000 });

		const snapRow = await env.DB
			.prepare("SELECT body, updated FROM auction_snapshot WHERE item='HYPERION'")
		...
		const point = await env.DB
			.prepare("SELECT tier, lowest, median, count FROM auction_points WHERE item='HYPERION'")
		...
		const meta = await env.DB.prepare("SELECT value FROM meta WHERE key='auctions_updated'").first<{ value: string }>();
		expect(meta!.value).toBe('1000000000000');
		...
		const updated = await env.DB.prepare("SELECT npc FROM items WHERE id='HYPERION'").first<{ npc: number }>();
	...
	test('a new never-tracked official id colliding with an existing slug is skipped, run completes', async () => {
		await env.DB
			.prepare('INSERT INTO items (id, slug, name) VALUES (?, ?, ?)')
			.bind('FOO_BAR', 'foo-bar', 'Foo Bar')
			.run();

		const stats: AuctionItemStats = { name: 'Other Item', tier: 'COMMON', lowestBin: 100, medianBin: 120, count: 3 };
		...
		const owner = await env.DB.prepare("SELECT name FROM items WHERE id='FOO_BAR'").first<{ name: string }>();
		expect(owner!.name).toBe('Foo Bar');
		// ...and every other write in the same run still landed.
		const good = await env.DB.prepare("SELECT category FROM items WHERE id='GOOD_ITEM'").first<{ category: string }>();
	...
	test('an aggregated auction id colliding with an existing slug still rejects loudly', async () => {
		...
		const stats: AuctionItemStats = { name: 'Colliding Data Item', tier: 'RARE', lowestBin: 500, medianBin: 550, count: 2 };
		await expect(writeAuctionRun(env.DB, 1_000_000_000_000, { 'BAZ-QUX': stats }, {})).rejects.toThrow();
	});
});
...
describe('rollupAll', () => {
	test('bazaar raw->hourly: rolls aged raw points into hourly medians and deletes them; idempotent', async () => {
		...
		await insertRaw();
		await rollupAll(env.DB, now);
		const hourlyAgain = (await env.DB.prepare('SELECT t, buy, sell FROM bazaar_points WHERE tier=1 ORDER BY t').all())
			.results as { t: number; buy: number; sell: number }[];
	...
	test('bazaar hourly->daily: rolls aged hourly points into daily medians and deletes them', async () => {
		...
		const pts = [0, 3 * HOUR, 20 * HOUR, 26 * HOUR].map((off, i) => ({ t: base + off, b: 10 + i, s: 5 + i }));
		await env.DB.batch(
			pts.map((p) =>
				env.DB
					.prepare('INSERT INTO bazaar_points (item, tier, t, buy, sell) VALUES (?, 1, ?, ?, ?)')
					.bind('WHEAT', p.t, p.b, p.s)
			)
		);

		await rollupAll(env.DB, now);
		const daily = (await env.DB.prepare('SELECT t, buy, sell FROM bazaar_points WHERE tier=2 ORDER BY t').all())
			.results as { t: number; buy: number; sell: number }[];
		const expected = bucketMedian(pts.map((p) => ({ t: p.t, b: p.b, s: p.s })), DAY, bazaarMedian);
	...
	test('auctions raw->daily: rolls aged raw points straight into daily medians, skipping an hourly tier', async () => {
		...
		// in the second.
		const pts = [0, 5 * HOUR, 30 * HOUR].map((off, i) => ({ t: base + off, l: 100 + i * 10, m: 150 + i * 10, c: 5 + i }));
		await env.DB.batch(
			pts.map((p) =>
				env.DB
		...
		await rollupAll(env.DB, now);
		const daily = (await env.DB.prepare('SELECT t, lowest, median, count FROM auction_points WHERE tier=2 ORDER BY t').all())
			.results as { t: number; lowest: number; median: number; count: number }[];
		const expected = bucketMedian(pts.map((p) => ({ t: p.t, l: p.l, m: p.m, c: p.c })), DAY, auctionMedian);
...
describe('pruneStaleSnapshots', () => {
	test('deletes snapshots older than a day, keeps fresh ones', async () => {
		const now = 2_000_000_000;
		await env.DB.batch([
			env.DB.prepare('INSERT INTO bazaar_snapshot (item, body, updated) VALUES (?, ?, ?)').bind('STALE', '{}', now - DAY - 1),
			env.DB.prepare('INSERT INTO bazaar_snapshot (item, body, updated) VALUES (?, ?, ?)').bind('FRESH', '{}', now - 10),
			env.DB.prepare('INSERT INTO auction_snapshot (item, body, updated) VALUES (?, ?, ?)').bind('STALE', '{}', now - DAY - 1),
			env.DB.prepare('INSERT INTO auction_snapshot (item, body, updated) VALUES (?, ?, ?)').bind('FRESH', '{}', now - 10)
		]);

		await pruneStaleSnapshots(env.DB, now);

		const bazaarLeft = await env.DB.prepare('SELECT item FROM bazaar_snapshot').all<{ item: string }>();
		expect(bazaarLeft.results.map((r) => r.item)).toEqual(['FRESH']);
		const auctionLeft = await env.DB.prepare('SELECT item FROM auction_snapshot').all<{ item: string }>();
